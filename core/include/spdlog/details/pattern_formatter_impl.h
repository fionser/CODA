...
class full_formatter :public flag_formatter
{
    void format(details::log_msg& msg, const std::tm& tm_time) override
    {
        ...
        // auto duration = msg.time.time_since_epoch(); // org
        // auto millis = std::chrono::duration_cast<std::chrono::milliseconds>(duration).count() % 1000; // org
        ...
        // msg.formatted << '[' << static_cast<unsigned int>(tm_time.tm_year + 1900) << '-' // org
                      // << fmt::pad(static_cast<unsigned int>(tm_time.tm_mon + 1), 2, '0') << '-' // org
                      // << fmt::pad(static_cast<unsigned int>(tm_time.tm_mday), 2, '0') << ' ' // org
                      // << fmt::pad(static_cast<unsigned int>(tm_time.tm_hour), 2, '0') << ':' // org
                      // << fmt::pad(static_cast<unsigned int>(tm_time.tm_min), 2, '0') << ':' // org
                      // << fmt::pad(static_cast<unsigned int>(tm_time.tm_sec), 2, '0') << '.' // org
                      // << fmt::pad(static_cast<unsigned int>(millis), 3, '0') << "] "; // org
...
//no datetime needed
        ...
        // msg.formatted << '[' << msg.logger_name << "] "; // org
#endif
